...
<div class="page-content">
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">User All </h6>
                        ...
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                ...
                                <tr id="user-row-{{ $item->id }}">
                                    ...
                                    <td>{{ $item->role }}</td>
                                    <td>
                                        <a href="{{ route('details.user', $item->id) }}" class="btn btn-inverse-info" title="View"><i data-feather="eye"></i></a>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    const statusLabel = $('#user-row-' + user_id).find('.status-label');
                    if (status == 'active') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkEcho = setInterval(function() {
        if (typeof window.Echo !== 'undefined') {
            ...
            // Listen for user status changes from other users
            window.Echo.channel('users')
                .listen('.status.changed', (e) => {
                    console.log('Received status change:', e);
                    
                    // Update the status badge
                    const statusLabel = $('#user-row-' + e.userId).find('.status-label');
                    if (e.status === 'active') {
                        statusLabel.removeClass('bg-danger').addClass('bg-success').text('Active');
                    } else {
                        statusLabel.removeClass('bg-success').addClass('bg-danger').text('Inactive');
                    ...
                    Swal.fire({
                        ...
                        text: 'A user status was updated by another user.',
                        toast: true,
                        position: 'top-end',
                        timer: 3000
                    });
            ...
            console.log('Echo connected - listening for user status changes');
        }
    }, 100);
});
</script>
