...
<style>
		...
		* {
		...
		.bee-row-3 .bee-row-content {
    background-color: #5b9fd6ff;  /* Changed from background-image to background-color */
...
<h1 style="color:#ffffff;direction:ltr;font-family:'Roboto Slab', Arial, 'Helvetica Neue', Helvetica, sans-serif;font-size:74px;font-weight:700;letter-spacing:normal;line-height:120%;text-align:left;margin-top:0;margin-bottom:0;">MyFixIt<br/> </h1>
</div>
<div class="bee-block bee-block-3 bee-paragraph">
<p>MyFixIt makes it easy to find trusted technicians for your repair and maintenance needs.<br/>Book verified professionals quickly and get your issues fixed with confidence.</p>
</div>
...
<div class="bee-block bee-block-1 bee-paragraph">
<p><strong>Dear {{ $booking['user']['name'] }},</strong><br/> 
@if($booking['status'] == '1')
<strong>Your Booking Request is Confirmed.</strong></p>
 @elseif($booking['status'] == '2')
<strong>Your Booking Request is Rejected.</strong></p>
 @elseif($booking['status'] == '3')
<strong>Your Booking Request is Cancelled.</strong></p>
 @endif

 @if($booking['status'] == '1')
<p>
    Your booking for the service {{ $booking['service']['service_name'] }} 
    on {{ $booking['booking_date'] }} at {{ $booking['booking_time'] }} 
    has been confirmed by the technician.
</p>

<p>
    Please note that a confirmation fee of 
    <strong>RM {{ number_format($booking['confirm_fee'], 2) }}</strong> 
    is required to after service is done.
</p>
    @elseif($booking['status'] == '2')
        <p>Your booking for the service {{ $booking['service']['service_name'] }} has been rejected. Reason: {{ $booking['rejection_message'] }}</p>
    @elseif($booking['status'] == '3')
        <p>Your booking for the service {{ $booking['service']['service_name'] }} on {{ $booking['booking_date'] }} at {{ $booking['booking_time'] }} has been cancelled by the technician. Reason: {{ $booking['cancellation_message'] }}</p>
    @endif

    <p>Best regards,<br>The MyFixIt  Team</p>
</div>
</div>
...
<p style="line-height: 18px; text-align: center;"><span style="line-height: 18px;">{{ $siteSettings->about }}</span></p>
</div>
</div>
</div>
...
<div class="bee-block bee-block-2 bee-text">
<div class="bee-text-content" style="line-height: 150%; font-size: 12px; font-family: inherit; color: #fafafa;">
<p style="line-height: 18px; text-align: center;"><span style="line-height: 18px;">{{ $siteSettings->company_address }}</span></p>
<p style="line-height: 18px; text-align: center;"><span style="line-height: 18px;">{{ $siteSettings->email }}</span></p>
<p style="line-height: 18px; text-align: center;"><span style="line-height: 18px;">+{{ $siteSettings->support_phone }}</span></p>
</div>
</div>
</div>
...
<p style="font-size: 14px; line-height: 16px; text-align: center;"><span style="font-size: 12px; line-height: 14px;">{{ $siteSettings->copyright }}</span></p>
</div>
</div>
</div>
</div>
